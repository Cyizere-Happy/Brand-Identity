...
const Football = () => {
  ...
  const bumpMap = useMemo(() => {
    ...
    texture.repeat.set(16, 8); // more stretching for football
    texture.needsUpdate = true;
    return texture;
  }, []);

  const material = useMemo(() => new THREE.MeshPhysicalMaterial({
    color: "#6B3E2E", // darker brown for leather
    roughness: 0.7,
    metalness: 0.1,
    bumpMap: bumpMap,
    bumpScale: 0.05,
  ...
  return (
    <group ref={meshRef}>
      ...
      <group position={[0, 0, 1.15]} rotation={[0, 0, 0]}>
        ...
        {/* Cross laces */}
        {[...Array(7)].map((_, i) => (
          <mesh key={i} position={[0, -0.45 + i * 0.15, 0.01]} rotation={[0, 0, Math.PI / 2]}>
      ...
      {/* Stretch the sphere to make it a football */}
      <mesh castShadow receiveShadow material={material} scale={[1, 1.4, 1]} rotation={[0, 0, Math.PI/2]}>
         <sphereGeometry args={[1.2, 128, 128]} />
      </mesh>
    </group>
  );
